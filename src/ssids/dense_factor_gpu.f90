...
subroutine node_ldlt(stream, nrows, ncols, gpu_L, gpu_LD, ldL, gpu_D, gpu_B,  &
  ...
  do while ( jb < ncols )
    ...
    if ( ib > right ) then ! the current partial elimination stage is over
      if ( right < ncols .and. left < done ) then
        !
    ...
    if ( pivoted < 1 ) then ! complete failure
      ...
      !
      do j = 1, cb
        if ( ind(j) < 1 ) cycle ! this column failed, leave it
        p = done + ind(j)
        q = ib + j - 1
        l = perm(q)
        perm(q) = perm(p)
  ...
  if ( ncols < nrows .or. done == ncols ) then
...
main_loop:&
  do
    ...
    do node = 1, nlvlnodes
      ncols = node_n(node)
      nrows = node_m(node)
      if ( jb(node) <= ncols ) ib(node) = jb(node) + 1
      if ( ib(node) > ncols ) cycle
      if ( ib(node) > right(node) ) then
        if ( right(node) < ncols .and. left(node) < done(node) ) then
...
subroutine node_llt(stream, nrows, ncols, gpu_L, ldL, gpu_B, block_size, &
  ...
  do while ( jb < ncols )
    ...
    if ( ib > right ) then
      ...
      if ( right < ncols .and. left < jb ) then
    ...
    if(cuda_error.ne.0) return
    if ( jb < nrows .and. jb < right ) then ! do forward elimination
...
end module spral_ssids_dense_factor_gpu
