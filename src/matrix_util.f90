...
!
  subroutine clean_cscl_oop_double(matrix_type, m, n, ptr_in, row_in, ptr_out,&
    ...
    if (matrix_type .lt. 0 .or. matrix_type .eq. 5 .or. matrix_type .gt. 6) then
       flag = ERROR_MATRIX_TYPE
...
!
  subroutine clean_cscl_oop_main(context, multiplier, matrix_type, m, n, &
       ...
       do col = 1, n
          ...
          if (i .ne. 0) then
             ...
             if (row_out(ptr_out(col)) .eq. col) then
                idiag = idiag + 1
             else if (abs(matrix_type) .eq. SPRAL_MATRIX_REAL_SYM_PSDEF) then
                flag = ERROR_MISSING_DIAGONAL
                call print_matrix_flag(context,nout,flag)
                call cleanup_dup(duphead)
                return
    ...
    if (present(val_out)) then
       ...
       case(SPRAL_MATRIX_REAL_SYM_PSDEF)
          ...
          do j = 1, n
             ...
             if (real(val_out(k)) .le. zero) then
                flag = ERROR_MISSING_DIAGONAL
                call print_matrix_flag(context,nout,flag)
                return
             end if
    ...
    if (ioor .gt. 0 .or. idup .gt. 0 .or. idiag .lt. n) then
       if (ioor .gt. 0) flag = WARNING_IDX_OOR
       if (idup .gt. 0) flag = WARNING_DUP_IDX
       if (idup .gt. 0 .and. ioor .gt. 0) flag = WARNING_DUP_AND_OOR
       if (abs(matrix_type) .ne. SPRAL_MATRIX_REAL_SKEW) then
          if (idiag .lt. n .and. ioor .gt. 0) then
             flag = WARNING_MISS_DIAG_OORDUP
          else if (idiag .lt. n .and. idup .gt. 0) then
             flag = WARNING_MISS_DIAG_OORDUP
          else if (idiag .lt. n) then
...
!
  subroutine convert_coord_to_cscl_double(matrix_type, m, n, ne, row, col, &
    ...
    if (abs(matrix_type) .eq. SPRAL_MATRIX_REAL_SYM_PSDEF) then
       do j = 1, n
    ...
    if (ioor .gt. 0 .or. idup .gt. 0 .or. idiag .lt. n) then
       ...
       if (abs(matrix_type).eq.SPRAL_MATRIX_REAL_SYM_INDEF .and. &
            idiag.lt.n .and. ioor.gt.0) then
          flag = WARNING_MISS_DIAG_OORDUP
       else if (abs(matrix_type).eq.SPRAL_MATRIX_REAL_SYM_INDEF .and. &
            idiag.lt.n .and. idup.gt.0) then
          flag = WARNING_MISS_DIAG_OORDUP
       else if (abs(matrix_type).eq.SPRAL_MATRIX_REAL_SYM_INDEF.and.idiag.lt.n) then
          flag = WARNING_MISSING_DIAGONAL
       end if
       call print_matrix_flag(context,nout,flag)
    end if
...
! Note: this is a modified version of mc34_expand from hsl_mc34
  subroutine half_to_full(n,row,ptr,iw,a)
    implicit none
