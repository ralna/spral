...
matrix of the form
\begin{equation}\label{nd:eqn:partition}
...
to compute the three sets that partition the matrix into the form (\ref{nd:eqn:partition}). The 
...
{\bf To compute the nested dissection ordering,}
   \vspace*{0.3cm} \\
   \texttt{\hspace*{0.2cm} call nd\_order(method,mtx,n,ptr,row,perm,options,inform[,seps])}
\vspace{0.3em}
...
\item[\texttt{method}] is an {\tt INTEGER} scalar with {\tt INTENT(IN)} that
...
\begin{description}
\item{\tt $=$ 1 } $\frac{n_s}{n_1 n_2};$
\item{\tt $=$ 2 } $n_s \left( 1 + \frac{\left|n_1 - n_2\right|}{2\left(n_1 + n_2 + n_s\right)} \right);$
\end{description}
...
\begin{description}
\item{\tt $=$ 1 } Use common neighbours matching (CNM) method.
\item{\tt $=$ 2 } Use sorted heavy-edge matching (SHEM) method.
\end{description}
The default is {\tt matching=2}.
...
\verbatiminput{examples/Fortran/nd.f90}
