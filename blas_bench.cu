...
int bench_nostream_syrk(int maxdimn, double *gpu_work) {
   char buffer[10000];
   cublasHandle_t cublas;
   cublasCreate(&cublas);
   ...
   while(!feof(stdin)) {
      ...
      if(blkm-blkn != 0 && blkn != 0)
         CublasSafeCall(
            cublasDsyrk(cublas, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
               blkm-blkn, blkn, &alpha, a, blkm-blkn, &beta, c, blkm-blkn)
            );
      flops += 2*(m*(m+1)*blkn)/2; // only do half matrix
   }
   cudaDeviceSynchronize();
   clock_gettime(CLOCK_REALTIME, &end);
   printf("nostream: Did %e flops\n", (float) flops);
   printf("nostream: Took %f seconds\n", tdiff(start, end));
   printf("nostream: Rate %f Gflop/s\n", flops/(1e9*tdiff(start,end)));
...
int bench_stream_syrk(int maxdimn, double *gpu_work) {
   char buffer[10000];
   cudaStream_t stream[16];
   for(int i=0; i<16; i++) CudaSafeCall( cudaStreamCreate(&stream[i]) );
   cublasHandle_t cublas[16];
   ...
   while(!feof(stdin)) {
      ...
      if(blkm-blkn != 0 && blkn != 0)
         CublasSafeCall(
            cublasDsyrk(cublas[s], CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
               blkm-blkn, blkn, &alpha, a, blkm-blkn, &beta, c, blkm-blkn)
            );
      flops += 2*(m*(m+1)*blkn)/2; // only do half matrix
      if(++s >= 16) s=0;
...
int main(void) {
   ...
   if(!bench_nostream_syrk(maxdimn, gpu_work)) { return 1; }
   //if(!bench_stream_syrk(maxdimn, gpu_work)) { return 1; }
