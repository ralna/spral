...
The packages :doc:`SPRAL_SSMFE <ssmfe>` and
:doc:`SPRAL_SSMFE_EXPERT <ssmfe_expert>` provide a
...
This package implements a block iterative algorithm for
...
The package :doc:`SSMFE <ssmfe>` provides a user-friendly interface to this
algorithm, whilst :doc:`SSMFE_EXPERT <ssmfe_expert>` provides an interface
that allows users to manage their own memory. If this routine is used instead
of :doc:`SSMFE_EXPERT <ssmfe_expert>`, the user is additionally responsible for
...
out-of-core and hybrid computation.
...
we refer to this workspace as a Fortran array ``W[kw+1][m][n]``, but the user
...
.. c:function:: void spral_ssmfe_core_default_options(struct spral_ssmfe_core_options *options)

   Intialises members of options structure to default values.

   :param options: Structure to be initialised.

.. c:function:: void spral_ssmfe_double(struct spral_ssmfe_rcid *rci, int problem, int left, int right, int m, double *lambda, double *rr, int *ind, void **keep, const struct spral_ssmfe_core_options *options, struct spral_ssmfe_inform *inform)
   ...
   * :math:`R` = ``rr[rci.k][rci.j:rci.j+rci.ny-1][rci.i:rci.i+rci.nx-1]``
   ...
   :param left: Number of left eigenpairs to find. On return with
...
.. c:function:: void spral_ssmfe_double_complex(struct spral_ssmfe_rciz *rci, int problem, int left, int right, int m, double *lambda, double complex *rr, int *ind, void **keep, const struct spral_ssmfe_core_options *options, struct spral_ssmfe_inform *inform)
...
.. c:type:: struct spral_ssmfe_core_options
...
.. c:type:: struct spral_ssmfe_infrom
   ...
   +--------------+-----------------------------------------------------------+
   |  -11         | left is out-of-range (:c:func:`spral_ssmfe_double`) or    |
   |              | nep is out-of-range                                       |
   |              | (:c:func:`spral_ssmfe_largest_double`).                   |
   +--------------+-----------------------------------------------------------+
...
.. literalinclude:: ../../examples/C/ssmfe/precond_core.c
   :language: C
...
The algorithm implemented by :c:func:`spral_ssmfe_double()` and
:c:func:`spral_ssmfe_largest_double()`
