...
!
module SPRAL_ssmfe_expert_double
  use SPRAL_ssmfe_core_double
  implicit none
...
contains
  ...
  subroutine ssmfe_inverse_rci_double &
    ...
    if ( rci%job == SSMFE_START .or. rci%job == SSMFE_RESTART &
      ...
      if ( rci%job /= SSMFE_QUIT ) then
        ...
        if ( keep%left_converged .and. keep%right_converged ) then
          ...
          if ( keep%left > 0 .and. keep%first > keep%left &
              .or. keep%right > 0 .and. block_size - keep%last >= keep%right &
              ) then
    ...
    case (SSMFE_SAVE_CONVERGED)
      ...
      if ( options%verbosity > 2 .and. rci%i < 0 ) then
...
!          do i = keep%lcon, nep + 1, -1
  ...
  subroutine ssmfe_vec_ops( n, m, X, ldX, W, ldW, V, rci, ind, U )
  ...
  subroutine ssmfe_msg( problem, options, left, right, m, flag )
    ...
    integer :: verbosity, max_it, err_est
    ...
    if ( verbosity <= NONE ) u_err = NONE
    if ( u_mon <= NONE .and. verbosity > NONE ) verbosity = 0
...
end module SPRAL_ssmfe_expert_double
