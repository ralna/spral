...
module spral_ssmfe_core_double
  ...
  interface ssmfe_solve 
    module procedure ssmfe_engine_double
  end interface 
  ...
  private :: lwork_evp, solve_evp, solve_gevp
...
if_rci: &
    if ( rci%job == SSMFE_START .or. rci%job == SSMFE_RESTART &
      ...
      if ( allocated(keep%work     ) ) deallocate ( keep%work      )
      if ( allocated(keep%rwork    ) ) deallocate ( keep%rwork     )
      ...
      keep%lwork = lwork_evp( mm )
      allocate( keep%work(keep%lwork), keep%rwork(3*mm - 2), stat = i )
      if ( i /= 0 ) then
...
! deallocate auxiliary arrays
  ...
  integer function lwork_evp( n )
    ...
    lwork_evp = max(3*n - 1, (nb + 2)*n)
    
  end function lwork_evp

  subroutine solve_evp( job, n, a, lda, lambda, &
  ...
  subroutine solve_gevp( n, a, lda, b, ldb, lambda, &
                          lwork, work, rwork, info )
    ...
    rwork(1) = 0
    call dsygv( SSMFE_ITYPE, SSMFE_JOBZ, SSMFE_UPLO, &
                n, a, lda, b, ldb, lambda, &
                work, lwork, info )

  end subroutine solve_gevp
  
end module spral_ssmfe_core_double
