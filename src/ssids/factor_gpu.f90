...
! This version handles multiple roots below the level set L_th.
  ...
  subroutine assign_subtrees(nstream, nnodes, child_ptr, child_list, sparent, &
    ...
    L_th(1) = nnodes+1
    loadbalance = 0.0
    do while (loadbalance .lt. min_loadbalance .and. nbelow .gt. 0)
       ...
       do i = 1, n_lth
          ...
          stream = 1
          do j = 2, nstream
             if (stwork(j) .lt. stwork(stream)) stream = j
          end do
  ...
  subroutine sort_nodes(n_lth, L_th, nwork)
    ...
    do while (nxchg .gt. 0)
       ...
       do i = 1, n_lth-1
          n1 = L_th(i)
          n2 = L_th(i+1)
          if (nwork(n1)%work_below .lt. nwork(n2)%work_below) then
             ! swap n1 and n2
             L_th(i+1) = n1
             L_th(i) = n2
             nxchg = nxchg + 1
...
!
  subroutine presolve_lwork(nnodes, nodes, sptr, rptr, nlev, tree_le, &
    ...
    do i = 1, tree_le(nlev + 1) - 1
       ...
       if (iwork(ntiles,1) .eq. 0) ntlev = ntlev + 1 ! count new tile-width level
    ...
    do i = 1, tree_le(nlev + 1) - 1
       ...
       if (iwork(ntiles,1) .eq. 1) iwork(ntiles,2) = iwork(ntiles,2) + blkm*nelim
    end do
  ...
  subroutine presolve_second(stream, nnodes, nodes, sptr, rptr, nlev, tree_lev, &
    ...
    do lev = 1, nlev
       ...
       do j = tree_le(lev), tree_le(lev + 1) - 1
          ...
          if (tleve(i + 1) - tleve(i) .lt. 2) cycle
...
!
  subroutine rebuild_rlist_direct(n, nnodes, nodes, sptr, rptr, rlist, &
    ...
    do node = 1, nnodes
       if (child_ptr(node + 1) - child_ptr(node) .lt. 1) cycle
       nelim = nodes(node)%nelim
       ndelay = nodes(node)%ndelay
       blkn = sptr(node+1) - sptr(node) + ndelay
       blkm = int(rptr(node+1) - rptr(node)) + ndelay
       ...
       do ci = child_ptr(node), child_ptr(node + 1) - 1
          ...
          j = 0
          do i = 1, cblkm
             if (rlist_direct(nr + i - 1, 1) .le. nelim) then
                j = j + 1
          ...
          do i = 1, cblkm
             if (rlist_direct(nr + i - 1, 1) .gt. nelim) then
                j = j + 1
                rlist_direct(nr + j - 1, 2) = i
             end if
          end do
...
!
  subroutine solve_setup(stream, pos_def, sparent, child_ptr, child_list, n, &
    ...
    do lev = 1, fact_data%num_levels
       ...
       if (lev .gt. 1) fact_data%values_L(lev - 1)%lc_size = lcc_size
