...
which relegates less work to the user.
}
...
\fullpackagename\ relegates 
more computational tasks to the user's code than the packages
\simple\ and \advanced, which are built upon it.
Not only are the storage and operations on all vectors of size $n$
%the problem size, 
user's code responsibility, but also
the decision on 
...
{\bf
To compute specified numbers %amounts 
...
and their $B$-images,
where {\tt mep} is not less than the number of wanted eigenpairs.
...
%
\itt{problem} is an \intentin\ scalar of type {\tt INTEGER} that
...
that holds the number of wanted leftmost eigenvalues.
...
{\tt \solver\_keep} that holds private data. 
%
...
%that holds additional information about errors and warnings
...
The algorithm implemented by {\tt \packagename\_solve}
...
supplies the subroutine {\tt apply\_2d\_laplacian()}
