...
\subsubsection{Compressed Sparse Column (CSC) Format} \label{nestd:cscformat}
...
%to hold matrix data in CSC format (see Section~\ref{nestd:cscformat}).

%\item[\texttt{row(:)}] is an {\tt INTEGER} array of rank one with {\tt INTENT(IN)}
%and size  {\tt ptr(n+1)-1}. On a call to {\tt nestd\_lower}, it must be 
...
%\item[\texttt{ml\_bandwidth}] is a {\tt DOUBLE PRECISION} scalar and is only used if 
...
\vspace{-1.0em}
\begin{algorithmic}
\STATE {\bf recursive subroutine multilevel\_partition}($A,P$)
\STATE Input: symmetric matrix $A$ of order $n$
\STATE Output: permutation matrix $P$
