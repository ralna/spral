! examples/Fortran/scaling/auction_unsym.f90 - Example code for SPRAL_SCALING
program auction_scale_unsym_example
   use spral_scaling
   ...
   call auction_scale_unsym(m, n, ptr, row, val, rscaling, cscaling, options, &
      inform, match=match)
   if(inform%flag<0) then
      write(*, "(a, i5)") "auction_scale_unsym() returned with error ", &
         inform%flag
      stop
   endif
...
end program auction_scale_unsym_example
